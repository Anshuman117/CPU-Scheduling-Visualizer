...
export default function CPUSchedulerHacker() {
      ...
      processes.map((p) =>
        p.id === id
          ? {
              ...p,
              [field]: value,
              ...(field === "burstTime" ? { remainingTime: value } : {}),
            }
          : p
      )
    );
  ...
  const calculateMetrics = (completedProcesses) => {
    return {
      ...
      avgTurnaroundTime:
        ...
        completedProcesses.length,
      avgResponseTime:
        completedProcesses.reduce((sum, p) => sum + p.responseTime, 0) /
        completedProcesses.length,
    };
  };
    ...
    while (completed < n) {
      ...
      if (selected.remainingTime === 0) {
        ...
        selected.turnaroundTime =
          selected.completionTime - selected.arrivalTime;
    ...
    const animate = () => {
      if (barIndex < bars.length && !isPaused) {
        ...
        if (timeElapsed === currentBar.startTime) {
          ...
          setHistogramBars((prev) => [
            ...prev,
            { ...currentBar, isActive: true },
          ]);
        }

        if (
          timeElapsed >= currentBar.startTime &&
          timeElapsed < currentBar.endTime
        ) {
          const progress =
            (timeElapsed - currentBar.startTime) / currentBar.targetLength;
          setHistogramBars((prev) =>
            prev.map((bar, idx) =>
              idx === barIndex
                ? { ...bar, currentLength: progress * bar.targetLength }
                : bar
            )
          );
        }
        ...
        if (timeElapsed === currentBar.endTime - 1) {
          setHistogramBars((prev) =>
            prev.map((bar, idx) =>
              idx === barIndex
                ? { ...bar, isActive: false, currentLength: bar.targetLength }
                : bar
            )
          );
          barIndex++;
  ...
  const metrics = calculateMetrics(
    processes.filter((p) => p.completionTime > 0)
  );
      ...
      <div className="min-h-screen bg-black text-green-400 font-mono overflow-hidden">
        ...
        <div className="relative z-10 h-screen flex">
          ...
          <div
            ...
            } bg-gray-900/90 backdrop-blur-sm border-r border-cyan-500/30`}
          >
                    ...
                    <CardContent>
                      <div className="space-y-3 max-h-64 overflow-y-auto custom-scrollbar">
                        {processes.map((process, index) => (
                          <div
                            key={process.id}
                            className="p-3 bg-black/30 rounded border border-gray-700/50 hover:border-purple-500/50 transition-colors"
                          >
  ...
  <span className="text-cyan-400 border-2 border-cyan-400 rounded-sm px-1 py-0.5">
    SCHEDVIEW
  </span>
    ...
    {" - CPU SCHEDULING VISUALIZER"}
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-yellow-400 font-mono">
                      TIME: {currentTime.toString().padStart(3, "0")}
                    </span>
                    {currentExecutingProcess && (
                      <>
                        ...
                        <span className="text-green-400 font-mono">
                          EXECUTING: {currentExecutingProcess}
                        </span>
                      </>
                    )}
                  </div>
            ...
            <div className="flex-1 p-6 overflow-auto">
              <Card className="bg-black/50 border-cyan-500/30 h-full overflow-hidden">
                <CardHeader>
                          ...
                          <div className="relative h-16 bg-black/30 rounded border border-gray-700/50">
                            {histogramBars.map((bar, index) => (
                              <Tooltip key={index}>
                                <TooltipTrigger asChild>
                                  <div
                                    className={`absolute top-2 bottom-2 rounded flex items-center justify-center font-bold text-black transition-all duration-300 border-2 ${
                                      bar.isActive ? "animate-pulse" : ""
                                    }`}
                                    style={{
                                      left: `${
                                        (bar.startTime / maxTime) * 100
                                      }%`,
                                      width: `${
                              ...
                              <Tooltip key={index}>
                                <TooltipTrigger asChild>
                                  <div className="flex flex-col items-center gap-2">
                                    <div
                                      className={`w-12 bg-gradient-to-t rounded-t transition-all duration-500 border-2 ${
                                        bar.isActive
                    ...
                    <div className="h-full flex items-center justify-center text-gray-500">
                      <div className="text-center">
                        ...
                        <div className="text-sm mt-2">
                          Press EXECUTE to start visualization
                        </div>
                      </div>
                    </div>
                  )}
          ...
          <div
            ...
            } bg-gray-900/90 backdrop-blur-sm border-l border-cyan-500/30`}
          >
            <div className="p-4 h-full overflow-y-auto">
              <div className="flex items-center justify-between mb-6">
                <div
                    ...
                    <CardContent className="space-y-3 text-xs">
                      ...
                      <div>
                        ...
                        <div className="text-cyan-400 font-mono">
                          {ALGORITHM_INFO[algorithm]?.timeComplexity}
                        </div>
                      </div>
                      <div>
                      ...
                      <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 gap-3">
                          ...
                          <div className="p-3 bg-cyan-600/10 rounded border border-cyan-500/30">
                            ...
                            <div className="text-xl text-cyan-300 font-mono">
                        ...
                        <table className="w-full text-xs font-mono">
                          <thead>
                            <tr className="border-b border-gray-700">
                              <th className="text-left p-1 text-purple-400">
                                PID
                              </th>
                              <th className="text-left p-1 text-purple-400">
                                AT
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              ...
              <div className="space-y-4">
                ...
                <p className="text-gray-400 text-sm leading-relaxed">
                  CPU Scheduler Visualizer – An interactive tool to visualize
                  and understand CPU scheduling algorithms in real time.
                </p>
                <div className="flex items-center gap-2 text-xs text-gray-500">
                  <span>Made with</span>
                  <Heart className="w-3 h-3 text-red-400 animate-pulse" />
                  <span>BY Anshuman & Krishna</span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <a
                    href="https://github.com/Anshuman117/CPU-Scheduling-Visualizer"
                    target="_blank"
                  ...
                  <a
                    href="mailto:anshumanhoon7@gmail.com?subject=CPU%20Scheduler%20Visualizer%20Issue"
                    className="flex items-center gap-2 text-gray-400 hover:text-cyan-400 transition-colors text-xs font-mono"
                  >
                ...
                <div className="pt-4 border-t border-gray-800">
                  <h5 className="text-xs font-bold text-gray-500 font-mono mb-2">
                    TECH_STACK
                  </h5>
